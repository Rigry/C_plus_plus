   ...
   while (qty--) {
      ...
      if (command == "CHANGE_CAPITAL") {
         ...
         if (capitals.count(country) == 0) {
            ...
            << country << " with capital " << capital << std::endl;
         ...
         else if(capitals[country] != capital)
         {
            ...
            << " has changed its capital from " << old_capital << " to " << capital << std::endl;
         }
         capitals[country] = capital;
      } 
      ...
      if (command == "RENAME") {
         ...
         if (old_country == new_country or capitals.count(old_country) == 0 or capitals.count(new_country) == 1)
            std::cout << "Incorrect rename, skip" << std::endl;
         else 
         {
            std::string old_country_capital {capitals[old_country]};
            capitals.erase(old_country);
            capitals[new_country] = old_country_capital;
            std::cout << "Country " << old_country << " with capital " << capitals[new_country]
      ...
      if (command == "ABOUT") {
         std::string country_;
         std::cin >> country_;
         if (capitals.count(country_))
            std::cout << "Country " << country_ << " has capital " << capitals[country_] << std::endl;
         else 
          std::cout << "Country " << country_ << " doesn't exist" << std::endl;
...
//     DUMP — вывод столиц всех стран.Формат ввода
